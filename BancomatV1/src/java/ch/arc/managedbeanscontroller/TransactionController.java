...
import ch.arc.ejbsessionfacade.TransactionFacade;
...
public class TransactionController implements Serializable {
    ...
    @EJB
    private ch.arc.ejbsessionfacade.TransactionFacade ejbFacade;
    ...
    public List<Transaction> findAllByUserId(){
        ...
        List<Transaction> transaction= ejbFacade.findAll();
        List<Transaction> transaction_= new ArrayList<>();
        for(int i = 0; i < transaction.size(); i++){
            if(transaction.get(i).getAccountId().getUserId().getId() == (Integer) session.getAttribute("userId")){
                transaction_.add(transaction.get(i));
            }
        }
        return transaction_;
    }
